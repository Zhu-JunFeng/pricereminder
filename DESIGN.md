...
全市场规则与单合约规则共用同一规则列表和编辑入口，但合约字段固定显示“全部 USDT 永续”。规则摘要必须直接表达“窗口内上涨或下跌达到阈值”，状态区单独展示全市场连接状态、有效合约数量和最后接收时间，避免用户把单合约行情连接误认为全市场扫描已运行。

### Windows Workspace

Windows 使用三页原生工作区：行情页突出当前价格和监控状态，预警页在同一工具带内切换涨跌幅与目标价格配置，设置页并排展示任务栏合约和连接自检。标签页只用海蓝绿下划线表达当前项；列表与输入区使用雾面层，不叠加装饰阴影。
...
- **Do** 使用系统亮暗主题与平台原生交互。
- **Do** 让通知内容完整解释命中的合约、窗口、阈值和实际变化；全市场提醒标题明确标识“全市场预警”。
