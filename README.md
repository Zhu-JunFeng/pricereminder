...
- **全市场异动提醒：** 配置一个时间窗口和涨跌阈值，自动扫描全部币安 USDT 永续合约；任一合约在窗口内达到阈值即提醒，无需逐个选择合约。
- **目标价格提醒：** 支持“达到或高于”和“达到或低于”两种方向。
- **开仓收益参考：** 每个合约可保存一组多单或空单开仓价，实时显示对应的未加杠杆价格收益率；盈利显示绿色，亏损显示红色。
...
开仓价仅用于终端展示，不会创建提醒或执行交易。每个合约保存一组方向和价格，旧版本中没有方向的开仓价会按多单读取：

```text
多单收益率 = (当前价格 - 开仓价格) / 开仓价格 × 100%
空单收益率 = (开仓价格 - 当前价格) / 开仓价格 × 100%
```

价格超过 30 秒未更新时保留最后一次计算结果，同时置灰并标记“价格已陈旧”。

### 全市场窗口异动

全市场规则使用币安 U 本位实时流中的最新成交价，支持 `1..60` 分钟和 `0.1%..100%`：

```text
上涨幅度 = (当前价格 - 窗口内最低价) / 窗口内最低价 × 100%
下跌幅度 = (当前价格 - 窗口内最高价) / 窗口内最高价 × 100%
```

- 不等待窗口积满；从启用后的第一条新价格开始，只要窗口内变化达到阈值就立即提醒。
- 每条规则、每个合约、上涨和下跌方向分别维护状态；触发后只重置本次触发的方向。
- 同一合约在同一时间命中多条规则或两个方向时合并为一条通知，不限制预先指定哪些合约。
- 低成交合约即使价格点较稀疏，只要两个价格点仍在配置窗口内，就会按实际时间和价格计算变化。
- 新建、编辑、重新启用、重新连接或前后台切换后均从第一条新价格开始，不用历史行情追溯触发。
- 全市场扫描只直连币安，不使用 PriceReminder 行情中继、其他交易所或备用数据源。
...
| --- | --- |
| 终端前台实时价格与本地规则 | 否 |
| 全市场扫描（Android / macOS / Windows / iPhone 前台） | 否，终端直连币安 |
...
```mermaid
flowchart LR
    A[终端 App] -->|指定合约优先连接| B[币安 U 本位 trade 流]
    A -->|全市场规则直连| G[币安 U 本位全市场实时价格流]
...
- 全市场流断开后清空滚动极值并自动重连，恢复后的第一条价格作为新基准。
...
| 模块 | 技术 |
| --- | --- |
| 共享规则语义 | Swift、Kotlin、C#、Go 分平台实现；各端核心检查覆盖阈值、滚动极值、方向独立重置、数据缺口与目标价格 |
...
- 只支持币安 U 本位永续合约；指定合约和全市场规则均使用最新成交价，不包含现货、交割合约、标记价格、K 线或其他交易所。
